    ...
    try:
        ...
        for c in products_category:
            try:
                ...
                main.conn.rollback()
                raise HTTPException(status_code=500, detail=f"Error fetching products for category: {str(e)}")
...
def filter_products_by_brand(product_brand: str):
    try:
        ...
        main.conn.rollback()
        raise HTTPException(status_code=500, detail=f"Error fetching products by brand: {str(e)}")
...
def filter_products_by_price(min_price: float = None, max_price: float = None):
    try:
        ...
        main.conn.rollback()
        raise HTTPException(status_code=500, detail=f"Error fetching products by price: {str(e)}")
...
def filter_products_by_color(color: str):
    try:
        ...
        main.conn.rollback()
        raise HTTPException(status_code=500, detail=f"Error fetching products by color: {str(e)}")
